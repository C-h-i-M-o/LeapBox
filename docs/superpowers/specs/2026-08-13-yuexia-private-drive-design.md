...
- 顶部只保留一个“上传”入口，点击后打开统一上传对话框。拖放区域通过 File System Entry API 自动识别文件与文件夹并递归读取；对话框同时保留“选择文件”和“选择文件夹”两个系统选择器，以兼容浏览器原生权限限制。
- “选择文件”使用 `<input type="file" multiple>`，一次可选择多个文件；“选择文件夹”使用 `webkitdirectory` 并保留目录层级。不支持目录 API 的浏览器回退到普通多文件上传。
- 上传队列的用户可见状态只显示“上传中”，不暴露“分片上传中”等内部实现术语；底层仍按 multipart 协议传输。
...
- 桌面侧栏保留我的文件、最近使用、收藏、回收站和空间摘要；顶部显示面包屑、搜索、新建文件夹、统一“上传”入口和视图切换。
...
- 桌面端表头和文件行共用同一组 CSS Grid 列定义；“操作”表头与每行操作区使用同一个末列并统一右对齐，不能依赖各自行宽估算。
- 中等宽度优先隐藏次要的类型或大小信息，但名称、更新时间和操作列保持稳定；调整窗口宽度时操作按钮不得挤入其他数据列。
- 手机端隐藏桌面表头，每个文件项使用两层紧凑布局：首层为复选框、图标与名称，次层为单行操作栏。收藏、下载、改名、移动和删除不得换行散落，删除操作不能单独掉到下一行。
- 手机端操作按钮的最小点击区域为 44 px；长文件名允许省略显示，但不得挤压操作栏或造成页面横向溢出。
...
- 使用 `gsap.timeline()` 编排首次进入：Logo 从匣子方向短促跃入，标题、工具区和当前可见文件项依次进入；文件项从左下方带轻微旋转回弹到位，最多只编排前 12 个可见项目。
- 主“上传”按钮点击时完成一次明显的上跃、轻微左右摆动和 `back.out` 回弹；导航切换时活动项从左侧滑入并轻微过冲，不能仅做不易察觉的透明度变化。
- 统一上传对话框按“背景淡入 → 面板从下方放大进入 → 文件图标跃入投放区”顺序播放。拖入或正在上传时文件图标可做节制的跃入循环，上传完成后以一次明显的回弹和成功闪烁收尾；空闲状态不播放无限装饰动画。
- 批量工具栏从底部用 `y` 与 `autoAlpha` 进入；选中文件时文件项水平偏移并轻微放大，复选框做一次回弹反馈。
- 对话框、上传中心和 Toast 使用可逆时间线；移动或删除前先播放清晰的退出动画，再更新列表状态。
- 文件移动或删除成功时以 `x`/`y`、`scale`、`autoAlpha` 退出后再移除；不动画 `width`、`height`、`top` 或 `left`。
- React 中使用 `useGSAP`、组件作用域引用和 `contextSafe` 管理生命周期；时间线在组件卸载时由上下文回收。只动画 `transform` 与 `autoAlpha`，仅对正在运动的元素短暂设置 `will-change`。
- `prefers-reduced-motion: reduce` 下跳过位移、旋转与回弹，只保留即时状态变化或不超过 100 ms 的淡化。
...
15. 顶部只显示一个“上传”入口；对话框可一次选择多个文件，也可选择文件夹，拖放时自动识别两者并按目录层级处理。
16. 上传队列显示“上传中”，不得出现“分片上传中”；文件选择按钮显示“选择文件”。
17. 桌面端“操作”表头与行内按钮右侧对齐；在 390 px 手机视口下，所有文件操作保持为一条稳定操作栏，删除按钮不单独换行。
18. Logo、上传按钮、导航切换、文件入场、选中、移动、删除和上传完成均有肉眼可辨的 GSAP 动效；减少动态效果模式仍能无位移完成全部操作。
