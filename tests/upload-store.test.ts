...
test("完成会话写入文件记录并将状态置为 completed", async () => {
  ...
  try {
    ...
    const second = await uploads.uploadPart(
      "owner-a", session.id, 2, new Uint8Array(1), 1,
    );
    const item = await uploads.completeSession("owner-a", session.id, [second, first]);
...
test("文件记录写入失败时清理已完成对象并终止会话", async () => {
  ...
  try {
    ...
    await assert.rejects(
      uploads.completeSession("owner-a", session.id, [part]),
      /上传记录保存失败/,
    );
